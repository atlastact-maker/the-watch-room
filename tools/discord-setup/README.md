...
| `support-desk.mjs` | A support ticket desk with no bot to host: builds `#support` under 📡 CONTROL where members cannot post to the channel but can open a **private thread** — visible only to them, the Commanders and The Duty Officer — and posts and pins the how-to. Every ticket is its own private room; archive it when sorted. Idempotent. **Superseded by `ticket-tool-prep.mjs` — Ticket Tool is being used instead.** Not deployed — it was built on the server on 2026-09-03 and removed the same day at the owner's request. `npm run support-desk` puts it back. |
| `ticket-tool-prep.mjs` | Server-side preparation for the Ticket Tool bot: a 🎫 TICKETS category `@everyone` cannot see (so a ticket channel is private the moment it is created, before the bot adds anyone), and a `#support` panel channel under 📡 CONTROL that members can read but not post in. Prints the channel and category IDs the tickettool.xyz dashboard asks for, and warns about the Ticket Tool role position and any missing permissions. Idempotent. |
| `advisor-service-split.mjs` | Makes the Advisory Wing per-service rooms a real permission boundary: a fire advisor sees `#fire-rescue` and not `#ambulance` or `#police`. Each room denies the base `Advisor` role (so the category-level allow cannot carry through) and allows only its own tag; Discord applies denies before allows, so holding the tag wins. Shared rooms are untouched. `--open` reverses it. **Supersedes `advisor-tags-access.mjs` for these five rooms — do not run that one after this.** Idempotent. |
...
| `suggestions-read.mjs` | Read-only. Pulls every thread in the `#suggestions` forum (open and archived) sorted by 👍 weight — title, tags, reactions, replies and the opening post — and with `--out` writes it as Markdown. `npm run suggestions` writes `suggestions.md` next to the script (gitignored); paste or drop that into a session for triage. Needs only View Channels + Read Message History. |
